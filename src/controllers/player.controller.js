...
export const myPlayerInfo = async (req, res, next) => {
    ...
    if (nowDirector.candidate_players.length === 0) {
        return res
            .status(404)
            .json({ message: 'candidate_player가 비어있습니다' });
    }
    //선수가 있다면
    ...
    const playerInArray = arrCandidatePlayer.includes(
...
export const playerUpgrade = async (req, res, next) => {
    ...
    try {
        ...
        if (
            upgradePlayer.player_unique_id !== materialPlayer.player_unique_id
        ) {
